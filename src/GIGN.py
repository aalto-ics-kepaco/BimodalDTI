...
class GIGN(nn.Module):
    def __init__(self, node_dim, hidden_dim, fc_input_dim: int | None = None):
        super().__init__()
        ...
        self.head = FC(fc_input_dim, hidden_dim, 3, 0.1, 1)

    def embed(self, data):
        return self.gnn(data)
    
    def decide(self, x: torch.Tensor) -> torch.Tensor:
        return self.head(x).view(-1)
    
    def get_latent(self, data):
        x = self.embed(data)
        return self.head.get_latent(x)
...
class FC(nn.Module):
    ...
    def get_latent(self, h):
        for i in range(len(self.predict) - 1):
            h = self.predict[i](h)
        return h
